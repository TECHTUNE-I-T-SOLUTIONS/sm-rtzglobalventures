  ...
  try {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          full_name: fullName,
          security_question_1: securityQuestions.question1,
          security_answer_1: securityQuestions.answer1,
          security_question_2: securityQuestions.question2,
    ...
    // Create profile record manually if the trigger doesn't work
    if (authData.user) {
      try {
        ...
        if (securityError) {
          ...
          // Don't fail the signup if security questions creation fails
        }
      } catch (dbError) {
