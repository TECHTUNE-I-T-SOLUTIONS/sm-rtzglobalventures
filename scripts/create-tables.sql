...
-- to create business_services table
...
CREATE POLICY "Users can manage own services" ON business_services FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Admins can view all services" ON business_services FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
);
...
-- to create functions for updated_at timestamps
...
-- to create triggers for updated_at
